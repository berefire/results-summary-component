...
function ResultScore({ score }) {
  ...
  return (
    <div className="w-full md:max-w-84.5 lg:max-w-92 bg-linear-to-b from-button-gradient-from to-button-gradient-to py-8 md:py-11.25 flex flex-col gap-6 md:gap-8 items-center rounded-b-4xl md:rounded-4xl font-body">
      ...
      <div className="flex size-35 md:size-50 flex-col items-center justify-center rounded-full bg-radial-[at_70%_10%] from-brand-gradient-from via-brand-gradient-to/60 to-transparent">
      <span className="sr-only">{`Score: ${score} out of 100`}</span>
        ...
        <span className="text-white/70 text-[1rem] md:text-[1.125rem] leading-[1.3]" aria-hidden="true">of 100</span>
      </div>
      <div className="flex flex-col items-center gap-4">
        <p className="text-white font-bold text-[2rem] leading-[1.3]">{label}</p>
        <p className="text-white/70 text-[1rem] md:text-[1.125rem] leading-[1.3] max-w-[30ch] md:max-w-[20ch] lg:max-w-[25ch] text-center">{message}</p>
      </div>
    </div>
  );
}
